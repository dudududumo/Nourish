...
export function CoreJourney({ hasBody, hasPlan, hasFeedback = false, onBody, onPlan }: { hasBody: boolean; hasPlan: boolean; hasFeedback?: boolean; onBody: () => void; onPlan: () => void }) {
  const steps = [
    ...
    { title: '执行反馈', detail: hasFeedback ? '今日执行反馈已记录' : hasPlan ? '记录执行，进入下一轮优化' : '执行后记录真实反馈', done: hasFeedback, action: onPlan, Icon: MessageSquareText },
  ];
